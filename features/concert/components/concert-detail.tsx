...
export function ConcertDetail({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
      <div className="relative w-full max-w-3xl rounded-3xl bg-zinc-900 border border-zinc-800 p-6 sm:p-8 text-zinc-100 shadow-2xl flex flex-col max-h-[90vh] my-6">
        {/* Header Bar */}
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              {concert.contractorName && (
                <span className="rounded-md bg-zinc-800/90 border border-zinc-700/60 px-2 py-0.5 text-[10px] font-medium text-zinc-300 inline-flex items-center gap-1">
                  <span>🏢</span>
                  <span className="truncate">{concert.contractorName}</span>
                </span>
              )}
            </div>
            <h2 className="text-xl sm:text-2xl font-bold text-zinc-50 truncate tracking-tight">
              {concert.title}
            </h2>
            <div className="flex items-center gap-3 mt-1.5 text-xs text-zinc-400 flex-wrap">
              ...
              {(concert.startTime || concert.finishTime) && (
                <span>
                  ⏰ {concert.startTime || "--:--"}
                  {concert.finishTime ? ` às ${concert.finishTime}` : ""}
                </span>
              )}
        ...
        {/* Action Toolbar */}
        <div className="flex items-center justify-between py-3 border-b border-zinc-800/80 flex-wrap gap-2">
        ...
        <div className="flex-1 overflow-y-auto py-2 divide-y divide-zinc-800/60 pr-1">
          {concert.setlist.length === 0 ? (
            ...
            concert.setlist.map((item, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3 rounded-xl transition-all group gap-3 ${
                    isDragging
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    {/* Drag Handle */}
                    <div
                      className="cursor-grab active:cursor-grabbing text-zinc-500 hover:text-zinc-300 px-1 py-1 select-none text-base"
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {/* Observação exibida na linha da música à frente do nome */}
                        {noteText && (
                          <span
                            className="rounded bg-zinc-800/90 border border-zinc-700/60 px-2 py-0.5 text-[11px] text-zinc-300 truncate max-w-[170px] sm:max-w-[240px] inline-flex items-center gap-1"
                            title={noteText}
                          >
                            <span>💬</span>
        ...
        {/* Footer */}
        <div className="flex items-center justify-between pt-4 border-t border-zinc-800 mt-2 shrink-0">
          <button
            type="button"
            onClick={() => {
